...
// access to http server @aws_ec2_seni
const char* serverAddress = "http://34.205.206.137/esp32max30102/insert_data.php";
...
int dataState = LOW;  // ledState used to set the LED
...
void loop() {
  ...
  // Specify the server address and the content type
    ...
    if (beatAvg > 60){
      ...
      // if the LED is off turn it on and vice-versa:
        if (dataState == LOW) {
          ...
          client.publish(AWS_IOT_PUBLISH_TOPIC, jsonBuffer);String postData = "beat=" + String(beatAvg) + "&espo2=" + String(ESpO2) + "&time=" + String(myTime);
